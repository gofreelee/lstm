...
class Resnext29Test : public ::testing::Test {
  protected:
    virtual void SetUp() override {
        output_.resize(sizeof(Resnext29Output) / sizeof(float));
        input_ = RandomData(sizeof(*device_input_) / sizeof(float));
        auto state = RandomData(sizeof(*state_) / sizeof(float));
        expect_ = ReadAll("resnext29.out", "./data");
    ...
    Resnext29State *state_;
    Resnext29Input *device_input_;
};

TEST_F(Resnext29Test, TestSingleInfer) {
    enum {
        kLoop = 100,
    };
    ...
    for (int i = 0; i < kLoop; ++i) {
        CUDA_CHECK(cudaMemcpy(device_input_, input_.data(),
                              ...
                              cudaMemcpyHostToDevice));
        const Resnext29Output *output = Resnext29Infer(device_input_, state_);
