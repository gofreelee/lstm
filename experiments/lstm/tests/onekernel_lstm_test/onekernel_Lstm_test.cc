#include "../../net/onekernel_lstm/OneKernel.h"
#include "../LstmTest.h"
...
TEST_F(LstmTest, OneKernelNet_1_256) {
    ...
    int fd =
        open("data", O_CREAT | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);

    for (auto &i : W) {
        for (int j = 0; j < hidden_size * hidden_size; ++j) {
            void *ptr = (void *)(i + j);
            void *base = ptr;
            int writen_bytes = 0, onetime_bytes;
            while (writen_bytes != sizeof(float)) {
                onetime_bytes = write(fd, ptr, sizeof(float) - writen_bytes);
                if (onetime_bytes == -1 && errno == EINTR)
                    continue;
                writen_bytes += onetime_bytes;
                ptr = (char *)base + writen_bytes;
            }
        }
    }
