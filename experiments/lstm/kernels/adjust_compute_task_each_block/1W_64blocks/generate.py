def genarate_function_call():
    ...
    for i in range(109):
        ...
        if i >= 10 and i <= 99:
            fd.write("cudaLaunchKernel((void *)wave_compute_" + str(i) + ", dim3(640), dim3(256), (void **)arg_s, 0,stream);\n")
            fd.write("cudaLaunchKernel((void *)wave_solve_" + str(i) + ", dim3(80), dim3(256), (void **)arg_s, 0,stream);\n")
        if i >99 and i < 109:
            fd.write("cudaLaunchKernel((void *)wave_compute_" + str(i) + ", dim3("+ str(64 * (109 - i)) +"), dim3(256), (void **)arg_s, 0,stream);\n")
    ...
    for i in range(110):
        if i <= 9:
            ...
            for m in range(i + 1):
                ...
                fd.writelines("call_onekernel_compute_2_wi_uh_0(" + str(m) + ", " + str(i - m) + ");")
                fd.writelines("break;")
            for m in range(i + 1):
                fd.writelines("case " + str(m + i + 1) + ":")
                fd.writelines("call_onekernel_compute_2_wi_uh_1(" + str(m) + ", " + str(i - m) + ");")
                fd.writelines("break;")
            fd.write("}\n")
            fd.write("}")
        if i > 9 and i <= 99:
            ...
            while index < 10:
                ...
                fd.writelines("call_onekernel_compute_2_wi_uh_0(" + str(index) + ", " + str(i - index) + ");")
                fd.writelines("break;")
                index = index + 1
            ...
            while index < 10:
                ...
                fd.writelines("call_onekernel_compute_2_wi_uh_1(" + str(index) + ", " + str(i - index) + ");")
                fd.writelines("break;")
                index = index + 1
            fd.write("}\n")
            fd.write("}")
        if i > 99:
            ...
            for m in range(108 - i + 1):
                fd.writelines("case " + str(m) + ":")
                fd.writelines("call_onekernel_compute_2_wi_uh_0(" + str(i - 99 + m) + ", " + str(99 - m) + ");")
                fd.writelines("break;")
            for m in range(108 - i + 1):
                fd.writelines("case " + str(m + 108 - i + 1) + ":")
                fd.writelines("call_onekernel_compute_2_wi_uh_1(" + str(i - 99 + m) + ", " + str(99 - m) + ");")
                fd.writelines("break;")
            fd.write("}\n")
            fd.write("}")

def generate_structparam_function_define_v2():
    fd = open("wavefunction.cc", "w+")
    for i in range(110):
        if i <= 9:
            # for j in range(i + 1):
...
def generate_structparam_function_define_v3():
    for i in range(109):
        ...
        if i > 9 and i <= 99:
            ...
            index = 0
            while index < 10:
                fd.writelines("case " + str(index + 50) + ":")
            ...
            index = 0
            while index < 10:
                fd.writelines("case " + str(index + 70) + ":")
...
generate_structparam_function_define_v3()