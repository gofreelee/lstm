...
#pragma unroll 5
    ...
    if (lane_id == 0) {
        sum_cached_ptr[blockIdx.x * 32 + warp_id] = x;
    }
    __syncthreads();

    x = (warp_id == 0 && lane_id < num_warp)
            ? sum_cached_ptr[blockIdx.x * 32 + lane_id]
            : 0.0f;
...
template <unsigned int hidden_size>
__global__ void solve(const float *__restrict__ state_c,
...
template __global__ void solve<256>(const float *__restrict__ state_c,
