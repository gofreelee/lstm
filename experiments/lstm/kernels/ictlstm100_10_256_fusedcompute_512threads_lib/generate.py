...
def genarate_waves_defination():
    for i in range(110):
        ...
        if i <= 9:
            # for j in range(i + 1):
            fd.write(
                "__global__ void __launch_bounds__(512, 1)wave_compute_"+str(i)+"(" + "WaveInputParams *__restrict__ input, WaveModelParams *__restrict__ model,WaveOutputParams *__restrict__ output")
            fd.write("){")
            ...
            for m in range(i + 1):
                ...
                str(m) + ", " +
                str(i - m) + "* LstmScaleParams::kCellNumber10 + " + str(m) +", " +
                "LstmScaleParams::kColumsPerBlock32, LstmScaleParams::kHiddenSize256, LstmScaleParams::kInputSize256, 512, LstmScaleParams::kMask7);")
                fd.writelines("break;")
            fd.write("}\n")
            fd.write("}")
