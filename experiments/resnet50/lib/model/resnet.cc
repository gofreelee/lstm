  ...
  public:
    ...
    typedef struct {
        typename StateTraits<
            Conv<kBatchSize, kHeight, kWidth, kChannelIn, kChannelBase,
                 kConv1FilterHW, kConv1FilterHW, kConv1Padding, kConv1Padding,
                 kConv1StrideHW, kConv1StrideHW, false, true, true>>::StateType
            fuse_conv1_bn_relu_state;
        typename StateTraits<
            Conv<kBatchSize, kHeight, kWidth, kChannelBase, kChannelBase,
                 kConv2FilterHW, kConv2FilterHW, kConv2Padding, kConv2Padding,
                 kConv2StrideHW, kConv2StrideHW, false, true, true>>::StateType
            fuse_conv2_bn_relu_state;
        typename StateTraits<
            Conv<kBatchSize, kHeightOut, kWidthOut, kChannelBase, kChannelOut,
                 kConv3FilterHW, kConv3FilterHW, kConv3Padding, kConv3Padding,
                 kConv3StrideHW, kConv3StrideHW, false, true, false>>::StateType
            fuse_conv3_bn_state;
        typename StateTraits<
            Conv<kBatchSize, kHeight, kWidth, kChannelIn, kChannelOut,
                 ...
                 false>>::StateType downsample_conv_bn_state;
    ...
    static void Forward(const Tensor<kInputSize> *input, StateType *state) {
        ...
        Conv<kBatchSize, kHeight, kWidth, kChannelIn, kChannelBase,
             kConv1FilterHW, kConv1FilterHW, kConv1Padding, kConv1Padding,
             kConv1StrideHW, kConv1StrideHW, false, true,
             true>::Forward(input, &state->fuse_conv1_bn_relu_state);

        Conv<kBatchSize, kHeight, kWidth, kChannelBase, kChannelBase,
             kConv2FilterHW, kConv2FilterHW, kConv2Padding, kConv2Padding,
             kConv2StrideHW, kConv2StrideHW, false, true,
             true>::Forward(&state->fuse_conv1_bn_relu_state.output,
                            &state->fuse_conv2_bn_relu_state);

        Conv<kBatchSize, kHeightOut, kWidthOut, kChannelBase, kChannelOut,
             kConv3FilterHW, kConv3FilterHW, kConv3Padding, kConv3Padding,
             kConv3StrideHW, kConv3StrideHW, false, true,
             false>::Forward(&state->fuse_conv2_bn_relu_state.output,
        ...
        if constexpr (kNeedDownsample) {
            Conv<kBatchSize, kHeight, kWidth, kChannelIn, kChannelOut,
                 ...
                 kConvDownsamplePadding, kConvDownsamplePadding,
                 kConvDownsampleStrideHW, kConvDownsampleStrideHW, false, true,
                 false>::Forward(input, &state->downsample_conv_bn_state);
            BottleneckAdd<
                kBatchSize, kBatchSize * kChannelOut * kHeightOut * kWidthOut,
...
class BottleNeck {
  public:
    ...
    typedef struct {
        typename StateTraits<BasicBlock<kBatchSize, kHeight, kWidth, kChannelIn,
                                        kChannelBase, true, kStride>>::StateType
            block1_state;

        typename StateTraits<
            BasicBlock<kBatchSize, kHeightOut, kWidthOut, kChannelOut,
                       kChannelBase, false, kRepeatBasicBlockStride>>::StateType
    ...
    static void Forward(const Tensor<kInputSize> *input, StateType *state) {
        ...
        BasicBlock<kBatchSize, kHeight, kWidth, kChannelIn, kChannelBase, true,
                   kStride>::Forward(input, &state->block1_state);

        for (unsigned i = 0; i < kRepeatNumber - 1; ++i) {
            if (i == 0) {
                BasicBlock<kBatchSize, kHeightOut, kWidthOut, kChannelOut,
                           kChannelBase, false, kRepeatBasicBlockStride>::
                    Forward(&state->block1_state.fuse_add_relu_state,
                            &state->blocks_state[0]);
            } else {
                BasicBlock<kBatchSize, kHeightOut, kWidthOut, kChannelOut,
                           kChannelBase, false, kRepeatBasicBlockStride>::
                    Forward(&state->blocks_state[i - 1].fuse_add_relu_state,
  ...
  public:
    ...
    typedef struct {
        typename StateTraits<
            Conv<kBatchSize, kResnet50ImageHeight, kResnet50ImageWidth,
                 ...
                 kConv1StrideHW, false, true, true>>::StateType
            fuse_conv1_bn_relu_state;
    ...
    static void Forward(const Tensor<kInputSize> *input, StateType *state) {
        ...
        BottleNeck<kBatchSize, kConv2BottleNeckInputHW, kConv2BottleNeckInputHW,
                   kConv2BottleNeckChannelIn, kConv2BottleNeckChannelBase,
                   kConv2BottleNeckRepeat, kConv2BottleNeckStride>::
            Forward(&state->maxpool1_state.output,
                    &state->conv2_bottleneck_state);

        BottleNeck<kBatchSize, kConv3BottleNeckInputHW, kConv3BottleNeckInputHW,
                   kConv3BottleNeckChannelIn, kConv3BottleNeckChannelBase,
                   kConv3BottleNeckRepeat, kConv3BottleNeckStride>::
        ...
        BottleNeck<kBatchSize, kConv4BottleNeckInputHW, kConv4BottleNeckInputHW,
                   kConv4BottleNeckChannelIn, kConv4BottleNeckChannelBase,
                   kConv4BottleNeckRepeat, kConv4BottleNeckStride>::
        ...
        BottleNeck<kBatchSize, kConv5BottleNeckInputHW, kConv5BottleNeckInputHW,
                   kConv5BottleNeckChannelIn, kConv5BottleNeckChannelBase,
                   kConv5BottleNeckRepeat, kConv5BottleNeckStride>::
