...
def genarate_waves_defination():
     for i in range(109):
        ...
        if i <= 9:
            ...
            fd.write("){")
            fd.write("switch (blockIdx.x >> 4) {\n")
            for m in range(i + 1):
                fd.writelines("case " + str(m) + ":")
                fd.writelines("call_onekernel_naivefuse_fusedsolve_fusedcompute_adduw_16blocks_eachcell_float4(" + 
                str(i - m) + "* LstmScaleParams::kCellNumber10 + " + str(m) +", " +
                str(m) + ", " +
                str(i - m) + "* LstmScaleParams::kCellNumber10 + " + str(m) +", " +
                "16, LstmScaleParams::kHiddenSize256, LstmScaleParams::kInputSize256, LstmScaleParams::kThreadNumPerBlock256, 15);")
                fd.writelines("break;")
            fd.write("}\n")
            fd.write("}")
